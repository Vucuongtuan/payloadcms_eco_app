...
export const ColumnMedia = ({ layout, items, columns, aspect }: any) => {
  const gridColumns = () => {
    let gridClass = "";
    switch (columns) {
      case 1:
  ...
  return (
    <div className={cn(layoutCtn(layout || "container"), gridColumns())}>
      {items.map((item: { media: MediaType; link: any }, idx: number) => (
        <Link
          href={item.link?.url || "#"}
          {...item.link?.newTabProps}
          key={idx}
          className={cn(`size-full relative group`, aspectConfig(aspect))}
        >
          <Media
            key={idx}
            resource={item.media}
